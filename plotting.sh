...
echo "set term pdf size 7,4 font 'Liberation Sans,18' noenhanced" > ${plotfilename}
...
echo "set key top left" >> ${plotfilename}
...
echo "pl '<(grep ^[0-9] ${file_to_plot})' u ${coltoplot} w l ${linespec} lc rgb 'black' title '${var_to_plot} (${colname})' \\" >> ${plotfilename}
if [ ! -z "${coltoplot2}" ]; then
	echo ", '<(grep ^[0-9] ${file_to_plot2})' u ${coltoplot2} w l ${linespec} lc rgb 'blue' title '${var_to_plot} (${colname2})' \\" >> ${plotfilename}
fi 
if [ ! -z "${coltoplot3}" ]; then
	echo ", '<(grep ^[0-9] ${file_to_plot})' u ${coltoplot3} w l ${linespec} lc rgb 'red' title '${var_to_plot3}' \\" >> ${plotfilename}
fi 
echo "" >> ${plotfilename}
